...
- 物品属性解析：支持从复制文本中提取品质、需求等级、物理/火焰/冰霜/闪电/混沌伤害范围、暴击率、攻击速度、护甲、闪避、能量护盾和插槽信息。
- 武器 DPS 计算：`physical_dps()`、`elemental_dps()`、`total_dps()`，缺失字段不会 panic。
- 挂单数据扩展：`TradeEntry` 新增价格数值、货币类型、基底类型、物品等级、在线状态、上架时间和私聊文本字段。
- 结构化排序：支持按价格、物品等级、上架时间和在线优先排序，使用数值比较而非字符串排序。
- 物品详情区：面板展示物品名称（按稀有度着色）、基底类型、基础属性、武器属性、DPS 汇总和防御属性。
- 纵向词缀筛选：完整的可交互词缀列表，支持三种状态（已选中/未选中/无法识别），点击词缀切换筛选，点击"重新搜索"统一发送请求。
- 挂单表格重做：6 列动态表格（等级 | 价格 | 状态 | 上架时间 | 卖家 | 私聊），交替行背景色，在线状态绿色/灰色圆点标识。
- 表头排序：点击价格/等级/时间表头切换排序方向，显示排序箭头指示器。
- 统一暗色主题：40+ 颜色常量集中管理，覆盖背景、文本、按钮、指标卡、表格行和私聊按钮。
- 按钮 hover 高亮：鼠标悬停时按钮背景/边框亮度提升 20。
- 鼠标滚轮支持：滚轮翻页，同时暂停自动隐藏倒计时。
- 鼠标悬停追踪：记录当前悬停控件，交互时暂停自动隐藏倒计时。

### Changed

- 代码结构重构：Overlay 相关代码从单文件 `main.rs`（~4700行）拆分为 `src/overlay/` 下 6 个模块（model、layout、theme、render、interaction、mod）。
...
- 标题栏按钮重构：固定/关闭/价值等级徽章移至标题栏右侧，紧凑排列，不再重叠。
